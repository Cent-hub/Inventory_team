<?php
/**
 * API Endpoint: Get Stock OUT Details or Listing
 * Primary Consumers: Team 3 (Production), Team 4 (Sales), Super Admin
 * Method: GET
...
$stockOutId = isset($_GET['id']) ? (int)$_GET['id'] : (isset($_GET['stock_out_id']) ? (int)$_GET['stock_out_id'] : 0);

try {
    if ($stockOutId > 0) {
        // Individual transaction lookup - enforces creator ownership
        $data = $service->getStockOutDetails($stockOutId, $authUser);
        jsonResponse([
            'success' => true,
            'data'    => $data
        ], 200);
        ...
        $list = $service->listStockOuts($authUser, $limit);
        jsonResponse([
            'success'     => true,
            'count'       => count($list),
